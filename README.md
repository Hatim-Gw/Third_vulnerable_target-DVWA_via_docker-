...
3. **Switched to DVWA via Docker.** Chose DVWA (Damn Vulnerable Web Application) as the replacement target (*official images exist with multi-arch (including arm64) support*) avoiding the architecture issue entirely.
4. **Pulled an ARM-compatible image.** Used `ghcr.io/digininja/dvwa:latest`, confirmed to support arm64.

5. **Ran the container**, mapping it to port 8080 on the host:

`
   docker run -d -p 8080:80 --name dvwa ghcr.io/digininja/dvwa:latest
`

6. **Verified locally first.** Loaded `http://localhost:8080` on the host, confirmed the DVWA login page rendered, logged in with default credentials, and initialized the database (Create/Reset Database).

7. **Confirmed reachability from Kali.** From the Kali VM, ran:
   `
